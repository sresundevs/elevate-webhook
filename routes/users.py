...
def handle_list_users():
    ...
    if len(filter["range"]) == 0:
        list_user = [
            ...
            for user in users.find({}, {"Password": 0}).sort("created_at", -1)
        ]
    else:
        ...
        list_user = [
            ...
            for user in users.find(
                {"created_at": {"$gte": dates[0], "$lte": dates[1]}}, {"Password": 0}
            ).sort("created_at", -1)
        ]
    return list_user
...
def handle_update_user():
    try:
        ...
        if (
            not userInfo["_id"]
            or not userInfo["Name"]
            or not userInfo["LastName"]
            or not userInfo["Email"]
        ):
            raise UserInfoException
        id = userInfo["_id"]
        userInfo.pop("_id")
        user = users.find_one_and_update(
            {"_id": ObjectId(id)}, {"$set": timestamps(userInfo, True)}
        )
        if not user:
            raise UserNotFoundException
...
def handle_set_password():
    try:
        ...
        if not userInfo["_id"] or not userInfo["Password"]:
            raise UserInfoException
        user = users.find_one_and_update(
            {"_id": ObjectId(userInfo["_id"])}, {"$set": timestamps(userInfo, True)}
        )
        if not user:
            raise UserNotFoundException
        resp = {"message": "User updated"}, 200
